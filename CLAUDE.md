...
templates/
  index.html         单页Web UI（Bootstrap 5 + 原生JS，五个Tab）
...
- 每行可展开查看评分维度明细（趋势强度/MACD动量/RSI区间/量能确认/布林带，含得分进度条和触发原因）
- 扫描结果可直接「+ 录入」到持仓台账
- 每次扫描结果自动持久化到数据库，可在「信号历史」Tab 回溯
...
- **删除持仓**：误录入时可删除，同时级联删除关联的复盘记录和交易流水
...
### Tab4 — 信号历史
- 下拉选择历史扫描日期（最近90天内有记录的日期）
- 显示当日市场环境及所有买入信号，格式与 Tab1 一致
- 每行可展开查看评分维度明细（复用 Tab1 同一套展示逻辑）

### Tab5 — 单只分析
- 输入任意 ETF 代码，可选指定分析日期（不填则分析最新数据）
- 结果显示：标的名称与当前价、综合评分、操作建议、当前市场环境
- 评分分解面板：5个维度得分进度条 + 触发原因（复用 Tab1 同一套展示）
- 指标卡片：均线排列状态、MACD 多空、RSI 区间、量能比、布林带位置、ATR
- 风控参考价位：止损（收盘价 − 2×ATR）、止盈（+3×ATR）、移动止损启动（+1×ATR）
- 持仓管理中代码列可点击，直接跳转到本 Tab 并自动触发分析
- 标的名称优先使用前端传入值，其次从 DB（scan_signals / positions）查找，最后降级为代码本身
- 指定日期分析时，后端将历史数据截断至该日期后重算指标（需该日期前至少30条数据）
...
### 辅助查询函数
- `get_etf_name(code)` — 按代码查 ETF 名称，先查 `scan_signals`（取最近扫描日），再查 `positions`，找不到返回 `None`
...
| score | 当日总评分 |
| price | 扫描时收盘价 |
| action | 信号类型（strong_buy / buy） |
| atr | 当日ATR |
| stop_loss_price / take_profit_price | 建议止损/止盈价 |
| market_regime / market_score | 当日市场环境 |
| details_json | 5个评分维度的详细得分（JSON） |

`(scan_date, code)` 唯一索引，同日重复扫描自动覆盖。
...
4. 止损检查（优先级由高到低）：
   - 当前价 ≤ 止损价 → `stop_loss`
   - 当前价 ≥ 止盈价 → `take_profit`
   - 持仓期最高价（`最高` 列）≥ 移动止损启动价 且 当前价 ≤ 最高价 - 1.5×ATR → `trailing`
...
### 移动止损计算细节
- **启动价**：`trailing_activate = buy_price + 1×ATR`（买入成本锚定，录入持仓时计算并固定）
- **追踪止损价**：`trailing_stop_price = recent_high - 1.5×initial_atr`（每次复盘实时计算）
- `recent_high` = 持仓日起至今每日 `最高` 价的最大值
- 日期过滤注意：Tushare `日期` 列格式为 `YYYYMMDD`，`buy_date` 为 `YYYY-MM-DD`，
  比较前需 `buy_date.replace("-", "")` 统一格式，否则字符串比较会使所有历史行通过筛选
...
详见 [STRATEGY.md](./STRATEGY.md)，包含数据源、技术指标、评分体系（5维度100分）、信号阈值、市场环境过滤的完整说明。
...
| `TRAILING_STOP_ATR_MULT` | 1.5 | 移动止损回撤ATR倍数（最高价 − 1.5×ATR） |
| `TRAILING_ACTIVATE_ATR_MULT` | 1.0 | 移动止损启动ATR倍数（买入价 + 1×ATR） |
...
| DELETE | `/api/positions/<id>` | 删除持仓（级联删复盘和流水） |
| GET | `/api/history` | 已平仓交易历史 |
| GET | `/api/scan_history/dates` | 有扫描记录的日期列表（最近90天） |
| GET | `/api/scan_history?date=` | 指定日期的扫描信号列表 |
| GET | `/api/analyze?code=&date=` | 单只ETF分析（date可选，不填分析最新数据） |
...
- 策略仅供研究参考，不构成投资建议
- Tushare `fund_daily` 返回的 `trade_date` 格式为 `YYYYMMDD`（无连字符），
  与界面传入的 `YYYY-MM-DD` 格式不同，凡涉及日期字符串比较须先统一格式
